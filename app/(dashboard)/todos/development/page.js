...
const DevelopmentTodosPage = async ({ searchParams }) => {

    const { status } = await searchParams

    const { all_count, filtered } = await getTodos("development", status)
    ...
    return (
        <div className="pt-6 max-w-7xl mx-auto">
            ...
            {all_count > 0 && <TodoFilter currentStatus={status} category="development" />}

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="sm:col-span-2">
                    <TodoListSection title="List" todos={filtered} />
                </div>
            </div>
        </div >
    );
