...
const DoneTodos = ({ title, todos }) => {
    ...
    return (
        <Card className="w-full shadow-none py-4 gap-4">
            ...
            <CardContent>
                {development.length > 0 && <DoneTodoList category="Development" todos={development} />}
                {personal.length > 0 && <DoneTodoList category="Personal" todos={personal} />}
                {work.length > 0 && <DoneTodoList category="Work" todos={work} />}
            </CardContent>
        </Card>
    )
}
